...
function Index() {
    ...
    <div className="flex flex-col gap-6">
      <div ref={panelsRef} className="flex flex-col gap-6">
        <details open className="panel group overflow-hidden">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-5 py-4 sm:px-6">
            <span className="text-sm font-bold">{recording ? `در حال ضبط… ${formatTime(elapsed)}` : "شروع ضبط و بارگذاری فایل"}</span>
            ...
            <div className="flex flex-col items-center gap-5">
              ...
              <div className="flex w-full flex-col items-center gap-4 border-t border-border pt-5 sm:flex-row sm:justify-between">
                <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border bg-surface px-4 py-2.5 text-sm font-medium text-surface-foreground transition-colors hover:bg-secondary">
                  <Upload className="size-4" /> آپلود صوت یا ویدیو
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={model} onChange={(e) => setModel(e.target.value)} disabled={loading} className="rounded-xl border border-border bg-card px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring">
                      {MODELS.map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
                    </select>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground">زبان خروجی:</span>
                    <select value={language} onChange={(e) => setLanguage(e.target.value as OutputLanguage)} disabled={loading} className="rounded-xl border border-border bg-card px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring" title="زبان متن خروجی">
        ...
        {audioUrl && (
          <details open className="panel group overflow-hidden">
            ...
            <div className="border-t border-border px-5 pb-5 pt-4 sm:px-6">
              <audio ref={playerRef} src={audioUrl} preload="metadata" onLoadedMetadata={(e) => setDuration(e.currentTarget.duration || 0)} onTimeUpdate={(e) => {
              ...
              <div dir="ltr" className="mb-1 flex items-center gap-3 text-xs font-mono text-muted-foreground">
                <span className="w-10 shrink-0 tabular-nums">{formatTime(currentTime)}</span>
                <input type="range" min={0} max={duration || 0} step={0.1} value={Math.min(currentTime, duration || 0)} onChange={(e) => seekTo(Number(e.target.value))} className="h-2 flex-1 cursor-pointer accent-primary" aria-label="موقعیت پخش" />
              ...
              <div dir="ltr" className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center">
                <div className="flex justify-end pr-3">
                  <button type="button" onClick={() => skip(-SKIP_SECONDS)} className="inline-flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-sm font-medium transition-colors hover:bg-secondary" title={`${SKIP_SECONDS} ثانیه عقب`}><SkipBack className="size-4" />{SKIP_SECONDS}</button>
                </div>
                ...
                <div className="flex justify-start pl-3">
                  <button type="button" onClick={() => skip(SKIP_SECONDS)} className="inline-flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-sm font-medium transition-colors hover:bg-secondary" title={`${SKIP_SECONDS} ثانیه جلو`}>{SKIP_SECONDS}<SkipForward className="size-4" /></button>
                </div>
              </div>
            </div>
          </details>
      ...
      {loading && (
        <div className="panel flex flex-col items-center justify-center gap-3 p-8 text-muted-foreground">
          <div className="flex items-center gap-3"><Loader2 className="size-5 animate-spin" /><span>{progressLabel || `در حال تبدیل «${fileName}» به متن…`}</span></div>
      ...
      {error && <div className="whitespace-pre-wrap rounded-2xl border border-destructive/30 bg-destructive/10 p-5 text-sm text-destructive">{error}</div>}
    </div>
  );
  ...
  const textPanel = hasTranscript ? (
    <div className="panel group flex flex-col overflow-hidden" style={lockedStyle}>
      <div className="flex shrink-0 items-center justify-between gap-3 px-5 py-4 sm:px-6">
        <span className="text-sm font-bold">خروجی متن ({segments.length} بخش){loading ? <span className="mr-2 text-xs font-normal text-muted-foreground"> (در حال تکمیل…)</span> : null}</span>
      </div>
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden border-t border-border px-5 pb-5 pt-4 sm:px-6">
        <div className="mb-3 flex shrink-0 items-center gap-2">
          <div className="relative flex-1">
            ...
            <input type="search" value={segmentQuery} onChange={(e) => setSegmentQuery(e.target.value)} placeholder="جستجو در جمله‌ها…" className="w-full rounded-xl border border-border bg-card py-2.5 pr-10 pl-3 text-sm outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </div>
        {filteredSegments.length === 0 ? (
          <p className="py-4 text-center text-sm text-muted-foreground">موردی یافت نشد.</p>
        ) : (
          <ul ref={listRef} className={`min-h-0 space-y-2 overflow-y-auto ${isDesktop && textLockH ? "flex-1" : "max-h-[22rem]"}`}>
  ...
  const analysisPanel = (
    <>
      {analysisError && <div className="whitespace-pre-wrap rounded-2xl border border-destructive/30 bg-destructive/10 p-5 text-sm text-destructive">{analysisError}</div>}
      {analysis && (
        <details open className="panel group overflow-hidden">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-5 py-4 sm:px-6">
            <span className="flex items-center gap-2 text-lg font-bold"><Sparkles className="size-5 text-primary" />{analysisMode === "full" ? "گزارش تحلیل کامل" : "تحلیل متن"}</span>
            <ChevronDown className="size-4 shrink-0 text-muted-foreground transition-transform group-open:rotate-180" />
          </summary>
          <div className="border-t border-border px-5 pb-5 pt-4 sm:px-6">
            ...
            <div className="whitespace-pre-wrap rounded-xl border border-border bg-surface p-4 text-base leading-9">{analysis}</div>
          </div>
        </details>
      )}
    </>
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-2.5 py-12 sm:px-3">
      <header className="text-center">
        <div className="flex justify-end"><ThemeToggle /></div>
        <h1 className="text-4xl font-black tracking-tight sm:text-5xl">VoicePluss</h1>
        <p className="mx-auto mt-4 max-w-xl text-base leading-8 text-muted-foreground">VoicePluss — ضبط یا آپلود صوت/ویدیو و دریافت متن فارسی یا انگلیسی. فایل‌های طولانی به‌صورت خودکار تقسیم و متن‌ها ادغام می‌شوند.</p>
      </header>
      {hasTranscript ? (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,0.95fr)]" dir="ltr">
          <div dir="rtl" className="order-2 flex flex-col gap-6 lg:order-1">{textPanel}{analysisPanel}</div>
          <div dir="rtl" className="order-1 lg:order-2">{controlsColumn}</div>
        </div>
      ) : (
